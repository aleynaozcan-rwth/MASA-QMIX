#!/usr/bin/env python3
"""
Full training visualization: All episodes grouped into 20-episode bins
Showing simultaneous vs conflicts distribution
"""
...
def plot_all_episodes_binned():
    ...
    # Stacked bars with color gradient showing progression
    # Simultaneous - gradient from lighter to darker blue (improving coordination)
    colors_sim = plt.cm.Blues(np.linspace(0.5, 0.8, len(simultaneous_means)))
    
    for i, (sim, color) in enumerate(zip(simultaneous_means, colors_sim)):
        ax.bar(x_pos[i], sim, width, color=color, alpha=0.85, 
              edgecolor='darkblue', linewidth=0.5)
    
    # Conflicts - gradient from dark red to light red (reducing conflicts)
    colors_conflicts = plt.cm.Reds(np.linspace(0.8, 0.3, len(conflicts_means)))
    
    for i, (conf, color) in enumerate(zip(conflicts_means, colors_conflicts)):
        ax.bar(x_pos[i], conf, width, bottom=simultaneous_means[i],
              color=color, alpha=0.85, edgecolor='darkred', linewidth=0.5)
    ...
    total_parallel = total_sim + total_conf
    sim_pct_of_parallel = 100 * total_sim / total_parallel
    conf_pct_of_parallel = 100 * total_conf / total_parallel
    
    # Calculate overall totals from raw data
    all_total_dps = sum([ep['total_coordination_dps'] for ep in episodes_data]) * len(episodes_data) / len(episodes_data)  # Approximate
    all_parallel = sum([ep['total_coordination_dps'] for ep in episodes_data])
    
    # Use known values from parallel_with_conflicts analysis
    total_dps_all = 39891
    simultaneous_all = 5553       # Pure simultaneity DPs (13.9%)
    conflicts_all = 3306          # Conflict retry DPs (8.3%)
    parallel_dps_all = simultaneous_all + conflicts_all  # 8859 (22.2%)
    
    title = 'Parallel Decision Points - Training Progression'
    ...
    legend_elements = [
        Patch(facecolor=plt.cm.Blues(0.5), edgecolor='darkblue', label='Early Simultaneous'),
        Patch(facecolor=plt.cm.Blues(0.8), edgecolor='darkblue', label='Late Simultaneous'),
        Patch(facecolor=plt.cm.Reds(0.8), edgecolor='darkred', label='Early Conflicts'),
        Patch(facecolor=plt.cm.Reds(0.3), edgecolor='darkred', label='Late Conflicts')
    ]
    legend = ax.legend(handles=legend_elements, loc='upper left', 
                      bbox_to_anchor=(0.01, 0.98), fontsize=10,
...
Job Arrival Rate: λ=0.125'''
    ...
    # Adjust layout to leave space on the right (same as before)
    plt.subplots_adjust(left=0.05, right=0.70, top=0.92, bottom=0.08)
    
    # Save
    output_path = OUTPUT_DIR / 'all_episodes_20bins_stacked.png'
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved all episodes visualization: {output_path}")
