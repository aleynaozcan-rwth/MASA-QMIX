...
"""Archived copy of utils/machine_registry.py

Preserved for history and reference. The canonical implementation now
lives in `utils.workcenter.WorkCenters.from_config`.
"""


# --- Begin archived content ---
"""Compatibility shim: delegate to `WorkCenters.from_config`.

This module used to contain a full `build_machine_registry` implementation.
That logic has been consolidated into `utils.workcenter.WorkCenters.from_config`.
Keeping this lightweight shim preserves backwards compatibility for any
callers that still import `build_machine_registry` while ensuring a single
...
try:
    ...
    WorkCenters = None  # type: ignore


def build_machine_registry(config: Dict[str, Any], strict_mode: bool = True) -> Tuple[WorkCenters, int, int]:
    """Return (WorkCenters instance, num_wcs, num_ops) by delegating to WorkCenters.from_config.

    This shim keeps backwards compatibility while the canonical implementation
    lives in `utils.workcenter`.
    """
    if WorkCenters is None:
        raise ImportError("utils.workcenter.WorkCenters not available")
    return WorkCenters.from_config(config, strict_mode)
