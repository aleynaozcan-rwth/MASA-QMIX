...
| processing_time_means | task_generator.py<br>workcenter.py | DEFAULT_PROCESSING_TIME_MEANS → self.proc_time_means in TaskGenerator<br>WorkCenters.create_decision_item (reads env.config['processing_time_means']) | Mean processing time per operation (Op1..OpN) per machine. Used to determine per-machine job durations and to fill per_machine_durations. |
...
Machine-level fields inside machines (e.g., wc, capable_ops, speed_factor) are parsed and normalized in WorkCenters.from_config() and used across environment and TaskGenerator logic.
...
utils/task_generator.py → DEFAULT_TASKGEN_PARAMS, DEFAULT_PROCESSING_TIME_MEANS
...
To run purely with in-code defaults, set auto_load_config=False or omit config_path.
